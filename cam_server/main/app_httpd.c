...
static int8_t LEDon=0;
static int8_t FlashOn=0;
...
static ra_filter_t * ra_filter_init(ra_filter_t * filter, size_t sample_size)
{
    ...
    if(!filter->values)
    {
        free(filter->values); // there was an error here!!
        return NULL;
    }
...
// capture the image from camera
	...
	if (FlashOn) gpio_set_level(4, 0); // turn led off
...
static esp_err_t cmd_handler(httpd_req_t *req)
{
	...
	// puts first "face_detect" then "face_recognize"to the OFF position on webpage if ->OFF, else turns LED on if FlashOn==off
        ...
        if (LEDon) 	
        {
			if (!FlashOn) gpio_set_level(4, 1); // turn led on
		}
...
static esp_err_t status_handler(httpd_req_t *req)
{
    ...
    *p++ = '{';
    ...
    p+=sprintf(p, ",\"LEDon\":%u", LEDon); // LEDon indication
    p+=sprintf(p, "\"FlashOn\":%u", FlashOn); // FlashOn indication
